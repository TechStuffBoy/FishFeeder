...
{
	if (i2c_start(address(iaddress,I2C_WRITE)))
		return 1;
...
{
	if (i2c_start(address(iaddress,I2C_READ)))
		return 1;
...
{
	i2c_start(address(devaddr,I2C_WRITE));
	i2c_write((uint8_t)regaddr);
	i2c_write(data);
	i2c_stop();
	return 0;
}
...
{
	if(MODE)
		ADDRESS = ( (uint8_t)ADDRESS << 1 ) | 0x01;
	else
		ADDRESS = ( (uint8_t)ADDRESS << 1 ) & 0xFE;
	return ADDRESS;
}
